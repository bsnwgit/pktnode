...
async def get_ip_info(ip: str, user: CurrentUser):
    ...
    async with httpx.AsyncClient(timeout=10) as client:
        await asyncio.gather(
            _fetch_ipinfo(client, ip, ipinfo_key, result),
            _fetch_ipapi_is(client, ip, ipapi_is_key, ipapi_is_free_tier, result),
            _fetch_abuseipdb(client, ip, abuseipdb_key, result),
            _fetch_mxtoolbox(client, ip, mxtoolbox_key, result),
        )
