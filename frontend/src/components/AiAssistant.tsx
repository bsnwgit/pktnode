 ...
 * Accessible from any page. Sends current view's node context to Claude.
 */
...
interface AiAssistantProps {
  /** Optional context from the current view */
...
export default function AiAssistant({ context = {} }: AiAssistantProps) {
  ...
  useEffect(() => {
    ...
    api.getSettings().then(s => {
      const key = s['anthropic_api_key'] as string
        ...
        <div className="fixed bottom-20 right-6 z-40 w-96 max-w-[calc(100vw-3rem)] bg-gray-900 border border-gray-700 rounded-2xl shadow-2xl flex flex-col"
          ...
          <div className="flex-1 overflow-y-auto px-4 py-3 space-y-3 text-sm">
            {configured === false && (
              <div className="bg-yellow-500/10 border border-yellow-500/30 rounded-lg p-3 text-yellow-400 text-xs">
                AI assistant not configured. Add your Anthropic API key in{' '}
                <strong>Settings → Security → AI Assistant</strong>.
              </div>
            )}
            {messages.length === 0 && configured !== false && (
              <div className="text-white text-xs space-y-2">
                <p>Ask anything about your nodes:</p>
                {[
                  'Why is this node showing offline?',
                  'Which nodes are running low on disk space?',
                  'What software is installed across my fleet?',
                ].map(q => (
                  <button
                    key={q}
                    onClick={() => { setInput(q); }}
          ...
          <div className="px-4 py-3 border-t border-gray-800 flex gap-2">
            <textarea
              ...
              placeholder={configured === false ? 'Configure API key first' : 'Ask about your nodes…'}
              rows={1}
