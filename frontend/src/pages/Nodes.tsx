...
export default function Nodes() {
  ...
  const selectableNodes = nodes.filter(n => n.status !== 'decommissioned')
  const allSelected = selectableNodes.length > 0 && selectableNodes.every(n => selected.has(n.id))

  const toggleAll = () => {
    setSelected(allSelected ? new Set() : new Set(selectableNodes.map(n => n.id)))
  }
  ...
  const bulkPowerAction = async (action: 'reboot' | 'shutdown') => {
    const targets = nodes.filter(n => selected.has(n.id))
    if (targets.length === 0) return
    const names = targets.map(n => n.display_name || n.hostname).join(', ')
    if (!confirm(
      `${action === 'reboot' ? 'Reboot' : 'Shut down'} ${targets.length} node${targets.length === 1 ? '' : 's'}?\n\n` +
      `${names}\n\n` +
      `This will ${action} each node the next time it checks in. ` +
      'Any unsaved work on those machines will be lost. Make sure that\'s intended.'
    )) return
        ...
        {loading ? (
          ...
          <table className="w-full text-sm">
              ...
              <tr className="border-b border-gray-800">
                {canAct && (
                  <th className="px-5 py-3 w-8">
                    <input type="checkbox" checked={allSelected} onChange={toggleAll} disabled={selectableNodes.length === 0} />
                  </th>
                )}
            ...
            <tbody className="divide-y divide-gray-800/50">
              ...
              {nodes.length === 0 && (
                <tr><td colSpan={8} className="px-5 py-8 text-center text-sm text-white">No nodes found</td></tr>
              )}
            </tbody>
          </table>
        )}
