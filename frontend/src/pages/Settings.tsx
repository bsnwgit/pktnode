...
import { api, getToken, User, UserIn, SslStatus, EnrollmentToken } from '../api/client'
...
// ── Main page ─────────────────────────────────────────────────────────────────
type TabId = 'general' | 'security' | 'data' | 'notifications' | 'enrollment'
...
const TABS: Array<{ id: TabId; label: string; adminOnly?: boolean; gapBefore?: boolean }> = [
  ...
  { id: 'enrollment',    label: 'Enrollment', adminOnly: true, gapBefore: true },
]
...
export default function Settings() {
  ...
  return (
    <div className="space-y-4">
            ...
            {securityTab === 'ai' && (
              <Section title="AI Assistant" onSave={aiAssistantSave.save} saving={aiAssistantSave.saving} saved={aiAssistantSave.saved} error={aiAssistantSave.error}
                help={{
                  ...
                  content: <>
                    <p><span className="text-gray-300 font-medium">AI Assistant</span> needs its own Anthropic API key (console.anthropic.com, separate from a Claude Enterprise seat) before the in-app chat panel does anything. Haiku is the default: fastest/cheapest for node and inventory questions.</p>
                  </>,
                }}
              >
      ...
      {/* Enrollment tab — admin only, app-specific */}
      {tab === 'enrollment' && isAdmin && <TabErrorBoundary><EnrollmentTab /></TabErrorBoundary>}
    </div>
  )
}
    ...
    <div className="space-y-4">
      <div className="flex items-center gap-2">
        ...
        <HelpButton title="Users — How It Works">
          <p>Three roles: <span className="text-gray-300 font-medium">admin</span> (full access, including this Users tab and Enrollment), <span className="text-gray-300 font-medium">analyst</span> (read access plus queuing remote actions), and <span className="text-gray-300 font-medium">viewer</span> (read-only).</p>
