...
`PKTNODE_REMOVE_EXISTING=1` (or `0`) to answer that prompt from a script;
...
encryption key), `pktnode.db` and its `-wal`/`-shm`, `logs/`, `backups/` and
anything uploaded under `ssl/`, `agent-releases/`. It asks separately before
