#!/bin/bash
# pktNode uninstall script — Ubuntu Server 22.04/24.04 LTS
...
# credential encryption key), pktnode.db and its -wal/-shm, logs, backups and
...
# WorkingDirectory, then $PKTNODE_INSTALL_DIR, then /opt/pktnode.

set -euo pipefail

APP="pktnode"
DISPLAY="pktNode"
UNIT_PATH="/etc/systemd/system/pktnode.service"
...
# migration — plain *.db does not match "pktnode.db.pre-upgrade".
...
if [ "$PURGE" -eq 1 ]; then
    for entry in logs backups ssl data agent-releases; do
    ...
    for entry in config.yaml pktnode.db logs backups ssl agent-releases; do
        if [ -e "$INSTALL_DIR/$entry" ]; then
            echo "    $entry"
        fi
    done
